...
public class StreamLoop<A> extends StreamWithSend<A> {
    ...
    public StreamLoop()
    {
    	if (Transaction.getCurrentTransaction() == null)
    	    throw new RuntimeException("lithium.StreamLoop/lithium.CellLoop must be used within an explicit transaction");
    }

    public void loop(Stream<A> ea_out)
    {
        if (this.ea_out != null)
            throw new RuntimeException("lithium.StreamLoop looped more than once");
        this.ea_out = ea_out;
