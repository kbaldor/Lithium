...
public class Stream<A> {
	private static final class ListenerImplementation<A> extends Listener {
		 ...
		 * the lithium.Listener, so that the finalizer doesn't get triggered.
		 */
    ...
    /**
     * Filter the empty values out, and strip the lithium.Optional wrapper from the present ones.
     */
